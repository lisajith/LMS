...
function AssignmentDetails() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const upload = await uploadAssignment(selectedFile);
      ...
      await setDoc(
        doc(db, "assignmentSubmissions", `${user.uid}_${assignment.id}`),
  ...
  if (!assignment) {
    return <div className="text-center py-24">Loading Assignment...</div>;
  }

  const dueDate = assignment?.dueDate
    ? typeof assignment.dueDate === "object" && assignment.dueDate.toDate
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center gap-3 text-theme-muted">
            ...
            <span>
              ...
              <span className="ml-2 font-semibold text-theme">
                {dueDate ? dueDate.toLocaleString("en-IN") : "No due date"}
              </span>
            </span>
          </div>
      ...
      <div className="card-theme rounded-2xl shadow p-8">
        <h2 className="text-2xl font-bold mb-5">Assignment Description</h2>

        <p className="leading-8 text-theme-muted">{assignment.description}</p>
      </div>
      ...
      <div className="card-theme rounded-2xl shadow p-8">
        <h2 className="text-2xl font-bold mb-6">Assignment File</h2>

        {assignment.attachmentUrl || assignment.attachment ? (
          <a
            href={assignment.attachmentUrl || assignment.attachment}
            target="_blank"
            rel="noreferrer"
          ...
          <p className="text-theme-muted">No assignment file available.</p>
        )}
      </div>
      {/* Submit Assignment */}
        ...
        {submission && !isExpired ? (
          ...
          <>
            <div className="rounded-xl border border-green-500 bg-green-50 p-5">
              <h3 className="font-bold text-green-600">Assignment Submitted</h3>
          ...
          <div className="mt-8 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-2xl font-bold">
                {submission.marks ?? 0}/{assignment.maxMarks}
              </span>
            </div>
